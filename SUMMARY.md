# Edu-Domains — Run Summary

_Generated 2026-09-15 21:58 UTC. Full per-country table: [data/countries/INDEX.md](data/countries/INDEX.md)._

## Totals

- Domains tracked: **1463** across **80** countries
- Active: **970** (66%) · Inaccessible: **493** (34%)
- Pending queue: **444** unvalidated candidates
- Known registration age: **799** domains (median 24 yrs)

## Last runs

- Verify (2026-09-15 21:46 UTC): validated 120, +76 Active, re-verified 15, pending left 328
- Curate (2026-09-15 21:58 UTC): 177 raw candidates, 116 queued, 6 re-cited

## Countries (top 15 by size)

| country | total | Active | Inaccessible |
|---|---|---|---|
| US | 525 | 408 | 117 |
| AU | 163 | 117 | 46 |
| FR | 149 | 92 | 57 |
| XX | 92 | 42 | 50 |
| AT | 41 | 35 | 6 |
| AM | 36 | 21 | 15 |
| GB | 34 | 22 | 12 |
| AL | 28 | 15 | 13 |
| CN | 27 | 11 | 16 |
| IN | 27 | 18 | 9 |
| NG | 27 | 19 | 8 |
| BY | 23 | 12 | 11 |
| DE | 17 | 10 | 7 |
| RU | 17 | 8 | 9 |
| BR | 16 | 10 | 6 |

## Validation signals (reasons in state detail)

| reason | count |
|---|---|
| `http-2xx-html` | 935 |
| `fetch-error:ConnectionError` | 131 |
| `http-403` | 101 |
| `low-confidence:http-2xx-html` | 66 |
| `fetch-error:ConnectTimeout` | 50 |
| `fetch-error:SSLError` | 36 |
| `empty-body` | 13 |
| `http-503` | 5 |
| `http-500` | 4 |
| `fetch-error:ReadTimeout` | 3 |
| `social-only/placeholder` | 3 |
| `http-405` | 2 |

## Freshness

- Verified in last 30 days: **1463** (100%)
- Older than 90 days: **0** (oldest check 2026-09-13)
- Archived chronic failures (re-check paused): **0**
- Moved pointers (old domain → new): **53**

## Alerts

- ⚠ `CD`: low Active rate 1/5 (20%) — check for blocks/stale sources
- ⚠ `CZ`: low Active rate 2/13 (15%) — check for blocks/stale sources
- ⚠ `IR`: low Active rate 0/16 (0%) — check for blocks/stale sources
- ⚠ `PH`: 4/13 blocked (403/429/block-page) — suspected bot-block, verify manually
- ⚠ `SD`: low Active rate 1/6 (17%) — check for blocks/stale sources
- ⚠ `XX`: 92 unmapped countries — expand `suffix_country` or fix source ISO

## Pipelines

- Curate hourly at :00 UTC (`src/curate.py`): upstream discovery → `state/pending.json`.
- Verify hourly at :30 UTC (`src/verify.py`): homepage checks + registration age → `data/countries/*.csv`.
- Schema: `school_name,web_domain,type,last_visited,status,sources,years_registered`.
- `status` is Active only on final HTTP 2xx + HTML + confidence with valid TLS; any non-2xx or TLS error is Inaccessible. `years_registered` is informational and never gates status.
