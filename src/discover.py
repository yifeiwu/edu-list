...
def discover_eter(url, state, per_run, timeout=180, max_mb: int = 100):
    # ETER full dump via Zenodo (~93MB CSV, `;` delimiter, CC-BY-4.0):
    # `English Institution Name`/`Institution Name`, `Institutional website`,
    # `Country Code`. Heavy download — disabled by default (see sources.yaml);
    # enable deliberately. Offset cursor over data rows.
    csv_url = url or ("https://zenodo.org/api/records/10879501/files/"
                      "ETER_fullDump_26032024.csv/content")
    try:
        r = _get(csv_url, timeout, stream=True)
        if r.status_code != 200:
            return []
        buf = io.BytesIO()
        size = 0
    ...
    if not rows:
        return []
    cols = {c.lower(): c for c in rows[0].keys()}
    web_c = cols.get("institutional website")
    name_c = cols.get("english institution name") or cols.get("institution name")
    cc_c = cols.get("country code")
    if not web_c or not name_c:
        return []
    off = _cursor(state, "eter_offset", 0)
    chunk = rows[off:off + per_run * 2]
    _advance_circular(state, "eter_offset", off, len(chunk), len(rows))
    out = []
    for row in chunk:
        web = (row.get(web_c) or "").strip()
        name = (row.get(name_c) or "").strip() or web
        iso = ""
        if cc_c:
            cc = (row.get(cc_c) or "").strip().upper()
            iso = cc if (len(cc) == 2 and cc.isalpha()) else ""
        if web and "." in web:
            out.append({"name": name, "url": web, "iso2": iso,
                        "type_hint": "university",
                        "source": f"eter:{time.strftime('%Y-%m')}"})
        if len(out) >= per_run:
            break
    return out
